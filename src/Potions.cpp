...
ItemOLD(name, descr, count, max_count, texture_file), heal_amount_(heal_amount)
{
...
ItemOLD* HealingPotion::clone()
{
	return new HealingPotion(*this);
}

HealingPotion::HealingPotion(const HealingPotion & orig) :
	ItemOLD(orig.name_, orig.description_, orig.count_, orig.max_count_, string(orig.texture_file_)), heal_amount_(orig.heal_amount_)
{
...
ManaPotion::ManaPotion(string name, string descr, size_t count, size_t max_count,
		...
		ItemOLD(name, descr, count, max_count, texture_file), restore_amount_(restore_amount)
{
...
ItemOLD* ManaPotion::clone()
{
	return new ManaPotion(*this);
}

ManaPotion::ManaPotion(const ManaPotion& orig) :
		ItemOLD(orig.name_, orig.description_, orig.count_, orig.max_count_, string(orig.texture_file_)),
